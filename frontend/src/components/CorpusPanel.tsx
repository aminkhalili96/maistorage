...
function SourceRow({ source, onPdf }: { source: SourceRecord; onPdf: (s: SourceRecord) => void }) {
  ...
  return (
    <div className="corpus-source-row">
      <div className="corpus-source-main">
        <a
          href={source.url}
          target="_blank"
          rel="noopener noreferrer"
          className="corpus-source-title"
          title={source.title}
        >
          {source.title}
        </a>
        <span className="corpus-doc-type">{docTypeLabel}</span>
      </div>
      {hasPdf && (
        <button
          className="corpus-pdf-btn"
          type="button"
...
function FamilyGroup({
  ...
  return (
    <div className="corpus-family-group">
      <button
        className="corpus-family-header"
        type="button"
        onClick={() => setOpen((o) => !o)}
      >
        <span className="corpus-family-chevron" style={{ transform: open ? "rotate(90deg)" : "rotate(0deg)" }}>›</span>
        <span className="corpus-family-label">{familyLabel(family)}</span>
        <span className="corpus-family-count">{sources.length}</span>
      </button>
      {open && (
        <div className="corpus-family-sources">
          {sources.map((s) => (
...
/* ── Main panel ── */
export function CorpusPanel({ open, onToggle }: { open: boolean; onToggle: () => void }) {
  ...
  return (
    <>
      ...
      <button
        className={`corpus-panel-tab${open ? " open" : ""}`}
        type="button"
        onClick={onToggle}
        title={open ? "Close corpus panel" : "Browse corpus"}
      >
        <DocsIcon />
        {!open && <span className="corpus-tab-label">Corpus</span>}
      </button>

      {/* Side panel */}
      <div className={`corpus-panel${open ? " open" : ""}`}>
        <div className="corpus-panel-header">
          <span className="corpus-panel-title">RAG Corpus</span>
          <span className="corpus-panel-subtitle">{sources.length} sources</span>
        </div>
        <div className="corpus-panel-scroll">
