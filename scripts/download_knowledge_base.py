...
def main() -> None:
    parser = argparse.ArgumentParser(description="Download the offline NVIDIA knowledge base bundle into data/knowledge_base.")
    parser.add_argument("--sources", default="data/sources/nvidia_sources.json")
    parser.add_argument("--knowledge-base-root", default="data/knowledge_base")
    ...
    knowledge_base_root = project_root / args.knowledge_base_root
    raw_html_root = knowledge_base_root / "raw/html"
    raw_pdf_root = knowledge_base_root / "raw/pdfs"
    normalized_root = knowledge_base_root / "normalized"
    ...
    (knowledge_base_root / "manifest.json").write_text(json.dumps(manifest, indent=2))
