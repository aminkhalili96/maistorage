...
def _load_questions() -> list[dict]:
    path = Path(__file__).resolve().parents[2] / "data" / "evals" / "jd_offline_extension_questions.json"
    return json.loads(path.read_text())


@pytest.fixture(scope="module")
def jd_extension_suite() -> tuple[list[dict], AgentService]:
    settings = replace(
        get_settings(),
...
def test_jd_extension_question_set_has_expected_coverage(jd_extension_suite: tuple[list[dict], AgentService]) -> None:
    questions, _ = jd_extension_suite
...
def test_jd_extension_topics_route_through_rag_with_citations(
    jd_extension_suite: tuple[list[dict], AgentService],
    index: int,
) -> None:
    questions, agent = jd_extension_suite
