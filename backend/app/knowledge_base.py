"""
Knowledge base data loaders — reads source registry, JSONL chunks, and manifest files.

Data layout:
  data/sources/nvidia_sources.json    → list of DocumentSource (49 sources)
  data/knowledge_base/normalized/*.jsonl → one file per source, each line is a ChunkRecord
  data/knowledge_base/manifest.json   → snapshot metadata (retrieved_at, content hashes)
  data/demo_chunks.json               → curated subset for fast Pinecone bootstrap
"""
...
def load_knowledge_base_manifest(path: Path) -> dict:
    if not path.exists():
        return {}
